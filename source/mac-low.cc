...
{
  //std::cout<<m_self<<" in set address\n";
  return m_self;
}
...
MacLow::ReceiveOk (Ptr<Packet> packet, double rxSnr, WifiTxVector txVector, bool ampduSubframe)
{
  ...
  double snrdb = 10*log10(rxSnr);

  //Mac48Address addrrx = hdr.GetAddr2();
  std::string s1 ="HtMcs";
  std::ostringstream mos;
  mos<<txVector.GetMode();
  ...
  if(m_self != Mac48Address("00:00:00:00:00:08") && hdr.GetAddr2() == Mac48Address("00:00:00:00:00:08") && s2.find(s1) != std::string::npos)//0d
  {
    ...
    addressandsnr[m_self]=snrdb;
    NS_LOG_DEBUG("GAGA The received pkts SNR is "<<snrdb<<"dB from "<<hdr.GetAddr2()<<" type is"<<hdr.GetType());
  }
...
std::map <Mac48Address, double> MacLow::fixedsnrperstation(void)
{
  return addressandsnr;

}

std::map <Mac48Address, uint32_t> MacLow::getpacketsizeperstation(void)
{
  return packetsizeperstation;
