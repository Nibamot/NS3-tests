  ...
  else
    {
      ...
      if (((m_currentHdr.IsQosData () && !m_currentHdr.IsQosAmsdu ())
      ...
      else
        {
          ...
          //printf("Every packet till here\n");
          //check PeekByTidAndAddress in mac-queue for details
          //std::cout<<m_queue->PeekByTidAndAddress (m_currentHdr.GetQosTid (),
            //                                       m_currentHdr.GetAddr1 ())<<" peek\n";
          if (m_currentHdr.IsQosData ()
              ...
              && m_msduAggregator != 0 && !m_currentHdr.IsRetry ())
            {
              ...
              if (isAmsdu)
                {
                  //std::cout<<"Address from:"<<m_currentHdr.GetAddr2()<<"\n";
                  count++;//hard
                  nooftxaggframesperstation[m_currentHdr.GetAddr1()] +=count;//hard
...
QosTxop::MissedAck (void)
{
  ...
  if (!NeedDataRetransmission (m_currentPacket, m_currentHdr))
    ...
    {
      noofretransmissionsbytes+=m_currentPacket->GetSize ();
      noofretransmissions++;
...
uint QosTxop::getnoofretransmssionsbytes(void)
{

return noofretransmissionsbytes;

}

uint QosTxop::getnoofretransmssions(void)
{
...
//hard
std::map <Mac48Address, uint32_t> QosTxop::getnooftxaggframesperstation(void)
{
