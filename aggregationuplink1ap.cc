...
// at this instant use function now()
  ...
  uint64_t simulationTime = 20; //seconds number of transmitted packets and in effect received packets vary with this
  ...
  fileme << "Stations,#Trial Number, Number of stations, Positionx,Positiony,Positionz,ResultantDistance,Payload,Aggregation size,MCS,Bitrate(kbps),Throughput,Goodput,Tx Packets,Rx Packets, Packet Loss Ratio, Mean Delay(ms), Channel Utilization\n";
...
//Enable A-MSDU with the highest maximum size allowed by the standard (7935 bytes)
  ...
  //fileme<<"AP,"<<wifiApNode.Get (0)->GetObject<Ipv4> ()->GetAddress (1, 0).GetLocal ()<<"\n";
    ...
    const auto addresse = ipv4->GetAddress (1, 0).GetLocal ();
    InetSocketAddress sinkSocket (addresse, port);
    ...
    for (uint32_t index = 0; index < nsta; ++index)
     ...
     for (uint32_t index = 0; index < nsta; ++index)
     {
       uint64_t totalbytesThrough = DynamicCast<PacketSink> (sinkApplications.Get (0))->GetTotalRx ();
     ...
     for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
     {
       if(staId<nsta)
       {
         ...
         fileme<<"Station "<<staId<<","<<trialno<<","<<nsta<<","<<positionxarr[staId]<<","<<positionyarr[staId]<<","<<positionzarr[staId]<<","<<distfromAP(positionxarr[staId],positionyarr[staId], positionzarr[staId])<<","
               <<payloadSize<<","<<BE_MaxAmsduSize<<","<<mcsvalue<<","<<datarate<<","<<throughput<<","<<goodput[staId]<<","<<txpkts<<","<<rxpkts<<","<<pktlossratio
