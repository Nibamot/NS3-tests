...
// at this instant use function now()
  ...
  uint64_t simulationTime = 4; //seconds number of transmitted packets and in effect received packets vary with this
  ...
  std::ofstream fileme;
  fileme.open("WalkRandomRFRaggregation.csv",std::fstream::app);
  fileme << "Stations,#Trial Number, Number of stations, Payload,Aggregation size,MostUsed AggregationSize, MCS, MostChosenMCS,Bitrate(kbps),Throughput,Goodput,Tx Packets,Rx Packets, Aggregated frames, Aggregated packets, retransmitbytes, Packet Loss Ratio, SNR (dB), Mean Delay(ms), Channel Utilization\n";
    ...
    for (uint32_t index = 0; index < nsta; ++index)
      {
       ...
       //uplink 2stations
           ...
           onOffHelperup.SetAttribute ("DataRate", DataRateValue (datarate/2.222));//bits
